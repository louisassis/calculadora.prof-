// incluir a outra parte do codigo (calculadora.h) que declara as funções
// Nessa outra parte do codigo tambem é declarada as bibliotecas 
...
void empilhar(Pilha *pilha, double valor)
{
    ...
    if (novoNo == NULL)// função verifica se a fila está vazia
    {
        printf("-------Erro: Falha na alocação de memória---------\n");
        exit(EXIT_FAILURE); // Saida com EXIT_FAILURE para informar que o codigo teve um retorno falho por não ter informação 
        }
//Nessa parte a função empilhar insere um novo item na pilha, realizando ajustes nos ponteiros proximo e Topo.
    novoNo->dado = valor;
    novoNo->proximo = pilha->topo;
    pilha->topo = novoNo;
}
//A função desempilha retira o item que está no topo de pilha
double desempilhar(Pilha *pilha)
{
    ...
    {// revisa para caso ocorra um erro de a pilha estar vazia 
...
// Mencionada antes no arquivo calculadora.h
double avaliarExpressaoPosfixada(const char *expressao)
{
    ...
    while (token != NULL)
    {
        ...
        }// ----- Avalia expressões importantes importantes como log, sen, cos e tg.======
                 ...
                 strcmp(token, "*") == 0 || strcmp(token, "/") == 0)
        {
            ...
            switch (token[0])
            {// faz um switch case para cada caso de expressão numerica 
            case '+':// soma
            ...
            case '*':// multiplicação
                empilhar(&pilha, operando1 * operando2);
                break;
            case '/':// e por ultimo a Divisão
                if (operando2 != 0.0)
                    empilhar(&pilha, operando1 / operando2);
                else
                {// tratamento de erro com a expressão numerica divisão 
                    printf("Erro: Divisão por zero\n");
                    exit(EXIT_FAILURE);
                }
                break;
            default:// outro tratamento de erro caso nenhum dos operadores numericos existentes tenham sido declarados 
...
// Implementação da função para converter expressão pós-fixada para prefixa
char *converterParaPrefixa(const char *expressao)
{
    ...
    while (token != NULL)
    {
        ...
        else
        {
            char *operando2 = desempilharStr(&pilha);
            char *operando1 = desempilharStr(&pilha);
            char *resultado = (char *)malloc(strlen(token) + strlen(operando1) + strlen(operando2) + 3);
            sprintf(resultado, "%s %s %s", token, operando1, operando2);
            empilharStr(&pilha, resultado);
            free(operando1);
            free(operando2);
...
int main()
{
    char expressaoPosfixada[] = "3 4 + 2 * 7 /";
    double resultado = avaliarExpressaoPosfixada(expressaoPosfixada);
    printf("Resultado da expressão pós-fixada '%s' = %f\n", expressaoPosfixada, resultado);

    char expressaoPosfixada2[] = "3 4 + 5 * 6 -";
    char *expressaoInfixa = converterParaInfixa(expressaoPosfixada2);
    printf("Expressão infixa da pós-fixada '%s' = %s\n", expressaoPosfixada2, expressaoInfixa);

    char *expressaoPrefixa = converterParaPrefixa(expressaoPosfixada2);
    printf("Expressão prefixa da pós-fixada '%s' = %s\n", expressaoPosfixada2, expressaoPrefixa);
